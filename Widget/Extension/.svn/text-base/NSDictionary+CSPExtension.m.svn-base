...
TT_FIX_CATEGORY_BUG(NSMutableDictionary_CSPExtension)
...
@implementation NSDictionary(CSPExtension)
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
    for ( Class clazzType = clazz; clazzType != [NSObject class]; )
    {
        ...
        for ( NSUInteger i = 0; i < propertyCount; i++ )
        {
            ...
            NSString *		propertyName = [[NSString stringWithCString:name encoding:NSUTF8StringEncoding] copy];
            const char *	attr = property_getAttributes(properties[i]);
            NSUInteger		type = [CSPTypeEncoding typeOf:attr];
            ...
            if ( tempValue )
            {
                ...
                {
                    NSString * className = [CSPTypeEncoding classNameOfAttribute:attr];
    ...
    SEL pathSelector = NSSelectorFromString(@"setPathProperties:");
    if ([object respondsToSelector:pathSelector]) {
        [object performSelector:pathSelector withObject:self];
    }
...
@implementation NSMutableDictionary(CSPExtension)
