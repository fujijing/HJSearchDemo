...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
    ...
    // return an sharedInstance of a MutableDictionary
...
//    }
    ...
    // know about the two sharedInstance methods used on that line; the code running on iOS will work well at run-time.
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    if (openUDID==nil) {
        if (mostReliableOpenUDID==nil) {
            // this is the case where this app sharedInstance is likely to be the first one to use OpenUDID on this device
